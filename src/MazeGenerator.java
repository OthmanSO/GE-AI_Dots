...
public class MazeGenerator {
	...
	/*
	 * void generate() { // Create a random maze. The strategy is to start with // a
	 * grid of disconnected "rooms" separated by walls, // then look at each of the
	 * separating walls, in a random // order. If tearing down a wall would not
	 * create a loop // in the maze, then tear it down. Otherwise, leave it in
	 * place. int i,j; int emptyCt = 0; // number of rooms int wallCt = 0; // number
	 * of walls int[] wallrow = new int[(rows*columns)/2]; // position of walls
	 * between rooms int[] wallcol = new int[(rows*columns)/2]; for (i = 0; i<rows;
	 * i++) // start with everything being a wall for (j = 0; j < columns; j++)
	 * maz[i][j] = wallCode; for (i = 1; i<rows-1; i += 2) { // make a grid of empty
	 * rooms for (j = 1; j<columns-1; j += 2) { emptyCt++; maz[i][j] = -emptyCt; //
	 * each room is represented by a different negative number if (i < rows-2) { //
	 * record info about wall below this room wallrow[wallCt] = i+1; wallcol[wallCt]
	 * = j; wallCt++; } if (j < columns-2) { // record info about wall to right of
	 * this room wallrow[wallCt] = i; wallcol[wallCt] = j+1; wallCt++; } } }
	 * 
	 * 
	 * int r; for (i=wallCt-1; i>0; i--) { r = (int)(Math.random() * i); // choose a
	 * wall randomly and maybe tear it down tearDown(wallrow[r],wallcol[r]);
	 * wallrow[r] = wallrow[i]; wallcol[r] = wallcol[i]; }
	 * 
	 * 
	 * 
	 * }
